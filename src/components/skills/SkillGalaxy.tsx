...
const SkillGalaxy: React.FC = () => {
    ...
    Object.entries(categoryGroups).forEach(
      ([category, catSkills], catIndex) => {
        ...
        catSkills.forEach((skill, index) => {
          ...
          const angle =
            startAngle + index * angleStep + angleVariation + Math.PI / 4;
  ...
  const connections = useMemo(() => {
    ...
    for (
      let i = 0;
      i < positions.length && conns.length < maxConnections;
      i++
    ) {
  ...
  return (
    <section className="skillGalaxySection w-full py-12 md:py-16 px-4 md:px-12">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Galaxy Container – vrij zwevend pannen */}
        <div
          className="relative w-full overflow-hidden rounded-2xl glass-card border border-white/10"
          style={{ height: "600px", touchAction: "none" }} // Belangrijk voor touch-panning
          ref={containerRef}
        >
          {/* Inhoud die we kunnen pannen – geen constraints, vrije beweging */}
          <motion.div
            className="absolute inset-0 cursor-grab active:cursor-grabbing"
            drag={true}
            dragElastic={0.1} // Iets elastisch voor natuurlijk gevoel
            dragMomentum={true} // Uitlopend effect na loslaten
          ...
          >
            ...
            {filteredSkills.map((skill, index) => {
              const pos = positions.find((p) => p.skillId === skill.id) || {
                x: 0,
                y: 0,
              };
