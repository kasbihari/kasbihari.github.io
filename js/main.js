/* =========
  ...
  /* =========
       About Page Specific JavaScript — Dropdown & Culture Slider
       ========= */
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Culture slider functionality - GEFIXTE VERSIE
    ...
    // Alleen initialiseren als er slides zijn
    if (cultureSlides.length === 0) return;
    
    function showSlide(index) {
      // Controleer of index binnen bereik is en slide bestaat
      if (index < 0 || index >= cultureSlides.length) return;
      if (!cultureSlides[index]) return;
      ...
      if (cultureDots[index] && cultureDots[index].classList) {
    ...
    // Add click events to dots (alleen als ze bestaan)
    ...
    // Add click events to navigation buttons (alleen als ze bestaan)
    if (prevBtn) {
    ...
    // Start auto-slide alleen als er meer dan 1 slide is
    if (cultureSlides.length > 1) {
      startAutoSlide();
      
      // Pause auto-rotation on hover
      const cultureSlider = document.querySelector('.culture-slider');
      if (cultureSlider) {
        cultureSlider.addEventListener('mouseenter', stopAutoSlide);
        cultureSlider.addEventListener('mouseleave', startAutoSlide);
        
        // Ook pause bij focus
        cultureSlider.addEventListener('focusin', stopAutoSlide);
        cultureSlider.addEventListener('focusout', startAutoSlide);
      }
    }
    ...
    // Initialize first passion card as open (alleen als er cards zijn)
    if (passionCards.length > 0 && passionCards[0]) {
